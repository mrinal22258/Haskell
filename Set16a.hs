...
isSorted :: (Ord a) => [a] -> Property
isSorted [] = property True
isSorted [x] = property True
isSorted (x:y:xs) = property (x <= y) .&&. isSorted (y:xs)
...
sumIsLength input output = property (sumOfFreqs output == length input)
  where
    sumOfFreqs :: [(a, Int)] -> Int
    sumOfFreqs [] = 0
    sumOfFreqs ((_, count):rest) = count + sumOfFreqs rest
...
-- | Property that checks if an element from the input appears in the output
inputInOutput :: (Show a, Eq a) => [a] -> [(a, Int)] -> Bool
inputInOutput input output = allElementsInOutput input output
  where
    allElementsInOutput :: (Eq a) => [a] -> [(a, b)] -> Bool
    allElementsInOutput input output = all (\x -> elementInPairs x output) input

    elementInPairs :: (Eq a) => a -> [(a, b)] -> Bool
    elementInPairs x pairs = any (\(y, _) -> x == y) pairs
...
outputInInput input output =
    forAll (elements input) $ \x ->
        property (all (\(y, n) -> countOccurrences y input == n) output)
  where
    countOccurrences :: Eq a => a -> [a] -> Int
    countOccurrences y xs = length [() | x <- xs, x == y]
