...
totalPrice (MkShoppingEntry _ price count) = itemTotalPrice price count
  where
    itemTotalPrice :: Double -> Int -> Double
    itemTotalPrice price count = price * fromIntegral count
...
buyOneMore (MkShoppingEntry name price count) = MkShoppingEntry name price (incrementCount count)
  where
...
extractX :: Position -> Int
extractX (Position x _) = x

extractY :: Position -> Int
extractY (Position _ y) = y
...
up (Position x y) = makePosition x (increaseY y)

-- right increases the x value of a position by one
right :: Position -> Position
right (Position x y) = makePosition (increaseX x) y
...
isIncreasing :: UpDown -> Bool
isIncreasing (Increasing _) = True
isIncreasing _              = False

isDecreasing :: UpDown -> Bool
isDecreasing (Decreasing _) = True
isDecreasing _              = False

getValue :: UpDown -> Int
getValue (Increasing n) = n
getValue (Decreasing n) = n
...
tick :: UpDown -> UpDown
tick ud
  | isIncreasing ud = Increasing (getValue ud + 1)
  | isDecreasing ud = Decreasing (getValue ud - 1)

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle ud
  | isIncreasing ud = Decreasing (getValue ud)
  | isDecreasing ud = Increasing (getValue ud)
...
mixColors [] [] = []
mixColors (x1:xs1) (x2:xs2) = ((x1 + x2) / 2) : mixColors xs1 xs2
...
toNat n
  | n < 0 = Nothing
  | otherwise = Just $ foldr (\_ acc -> PlusOne acc) Zero [1..n]
