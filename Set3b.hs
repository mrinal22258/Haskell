...
sums i = to 1 0
  where
    ...
    to n acc
      | n > i     = []
      | otherwise =  (acc + n) : to (addOne n) (acc + n)
...
indexDefault xs i def = indexHelper xs i def 0
  where
    indexHelper :: [a] -> Int -> a -> Int -> a
    indexHelper [] _ def _ = def
    indexHelper (x:xs) i def currentIndex
      | i < 0             = def
      | currentIndex == i = x
      | otherwise         = indexHelper xs i def (currentIndex + 1)
...
sorted [] = True
sorted [x]= True
sorted (x:y:xs) = not (x > y) && sorted (y:xs)
...
sumsOf xs = to 0 xs
  where
    to _ [] = []
    to sum (x:xs) = newSum : to newSum xs where newSum = sum + x
...
handleEmptyList :: (a -> a -> Bool) -> a -> [a] -> a
handleEmptyList _ initial [] = initial
handleEmptyList bigger initial xs = findMaximum bigger initial xs

updateMax :: (a -> a -> Bool) -> a -> a -> a
updateMax bigger currentMax x =
  if bigger x currentMax then x else currentMax

findMaximum :: (a -> a -> Bool) -> a -> [a] -> a
findMaximum _ currentMax [] = currentMax
findMaximum bigger currentMax (x:xs) =
  let newMax = updateMax bigger currentMax x
  in findMaximum bigger newMax xs

mymaximum :: (a -> a -> Bool) -> a -> [a] -> a
mymaximum bigger initial xs = handleEmptyList bigger initial xs
...
map2 :: (a -> b -> c) -> [a] -> [b] -> [c]
map2 f as bs = map2Helper f as bs
  where 
    map2Helper :: (a -> b -> c) -> [a] -> [b] -> [c]
    map2Helper _ [] _ = []
    map2Helper _ _ [] = []
    map2Helper f (x:xs) (y:ys) = f x y : map2Helper f xs ys
