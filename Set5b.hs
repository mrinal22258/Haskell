...
treeSize :: Tree a -> Int
treeSize tree = foldl (\count _ -> count + 1) 0 (toList tree)
  where
    toList :: Tree a -> [a]
    toList Empty = []
...
treeMax tree = foldl max 0 (toList tree)
  where
    toList :: Tree a -> [a]
    toList Empty = []
    toList (Node a left right) = toList left ++ [a] ++ toList right
...
allValues condition tree = foldl (\acc x -> acc && condition x) True (toList tree)
  where
    toList :: Tree a -> [a]
    toList Empty = []
    toList (Node a left right) = toList left ++ [a] ++ toList right
...
walk steps tree = case walkHelper steps tree of
    Just (Node value _ _) -> Just value
    _ -> Nothing

walkHelper :: [Step] -> Tree a -> Maybe (Tree a)
walkHelper [] tree = Just tree
walkHelper _ Empty = Nothing
walkHelper (StepL:steps) (Node _ left _) = walkHelper steps left
walkHelper (StepR:steps) (Node _ _ right) = walkHelper steps right
...
set [] newVal tree = case tree of
    Empty -> Empty
    Node _ left right -> Node newVal left right
set (step:steps) newVal tree = case tree of
    Empty -> Empty
    Node val left right -> case step of
        StepL -> Node val (set steps newVal left) right
        StepR -> Node val left (set steps newVal right)
...
search x tree = case tree of
    ...
    Node val left right
        | x == val -> Just []
        | otherwise -> case search x left of
            Just steps -> Just (StepL : steps)
            Nothing -> case search x right of
