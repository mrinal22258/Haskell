...
updateAt :: Int -> a -> [a] -> [a]
updateAt i x xs 
    | length xs  < i || i < 0  = error "No such position found!"
...
substring i j s 
    | i > length s || j > length s || i < 0 || j < 0 = error "No such position found!"
    | otherwise = take t (drop i s)
    where 
        t = j - i
...
removeFirstAndLast s
    | length s <= 2 = ""
    | otherwise  = take (length s - 2) (drop 1 s)

palindromify :: String -> String
palindromify s = if isPalindrome s then s else palindromify (removeFirstAndLast s)
  where 
    isPalindrome :: String -> Bool
    isPalindrome ""  = True
    isPalindrome str = str == reverse str
...
safeDiv x y = if y == 0 then Nothing else Just b
    where 
        b = x `div` y
...
getElementAt (x:xs) i
    | i < 0     = Nothing
    | i == 0    = Just x
    | otherwise = getElementAt xs (i - 1)

safeIndex :: [a] -> Int -> Maybe a
safeIndex xs i 
    | i < 0 || i >= length xs = Nothing
    | otherwise  = getElementAt xs i
...
eitherDiv x y = if y == 0 then Left (show x ++ "/0") else Right b
    where 
        b = x `div` y
