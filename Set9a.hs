...
workload nExercises hoursPerExercise =
  (\totalHours ->
     if totalHours > 100 then "Holy moly!"
     else if totalHours < 10 then "Piece of cake!"
     else "Ok."
  ) (nExercises * hoursPerExercise)
...
echo :: String -> String
echo str = concat [take (length str - i) (drop i str) ++ ", " | i <- [0..length str - 1]]
...
countValid serials = length [serial | serial <- serials, isValid serial]
  where
    isValid :: String -> Bool
...
repeated [] = Nothing
repeated (x:xs) = findRepeated xs x 1
  where
    findRepeated [] _ _ = Nothing
    findRepeated (y:ys) current count
      | y == current = if count == 1 then Just y else findRepeated ys current (count + 1)
      | otherwise    = findRepeated ys y 1
...
sumSuccess xs = case [x | Right x <- xs] of
  [] -> Left "no data"
  xs -> Right $ sum xs
...
open code lockState =
    case lockState of
        Closed correctCode ->
            if code == correctCode
                then Open correctCode
                else Closed correctCode
        openLock@(Open _) -> openLock
...
lock lockState =
    case lockState of
        ...
        closedLock@(Closed _) -> closedLock

-- changeCode changes the code of an open lock. If the lock is closed,
-- nothing happens.
changeCode :: String -> Lock -> Lock
changeCode newCode lockState =
    case lockState of
        Open _ -> Open newCode
        closedLock@(Closed _) -> closedLock
...
instance Eq Text where
  (Text s1) == (Text s2) = foldr (\c acc -> if isSpace c then acc else c : acc) [] s1 ==
                           foldr (\c acc -> if isSpace c then acc else c : acc) [] s2
------------------------------------------------------------------------------
...
compose f g = foldr (\(key, value) acc -> case lookup value g of
                        Just c -> (key, c) : acc
                        Nothing -> acc) [] f
