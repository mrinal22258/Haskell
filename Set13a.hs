...
checkNumber (forename, surname) = do
  let names = [forename, surname]
  mapM_ (guard . not . any isDigit) names
...
checkCapitals (forename, surname) = do
  let names = [forename, surname]
  mapM_ (guard . allStartsWithCapital) names
  return (forename, surname)
  where
    allStartsWithCapital str = not (null str) && isUpper (head str)
...
winner scores player1 player2 = do
  [score1, score2] <- mapM (\player -> lookup player scores) [player1, player2]
  if score1 >= score2
    then Just player1
    else Just player2
...
safeIndex :: [a] -> Int -> StateT [a] Maybe a
safeIndex xs idx = StateT $ \s ->
  if idx < 0 || idx >= length xs
    then Nothing
    else Just (xs !! idx, s)

selectSum :: Num a => [a] -> [Int] -> Maybe a
selectSum xs indices = evalStateT (sum <$> mapM (safeIndex xs) indices) xs
...
countAndLog p xs =
    let satisfying = [x | x <- xs, p x]
        logged = map show satisfying
    in Logger logged (length satisfying)
...
balance accountName = BankOp $ \(Bank accounts) ->
  let balance = case Map.lookup accountName accounts of
                  Just amount -> amount
                  Nothing     -> 0
  in (balance, Bank accounts)
...
count x = state $ \stateList ->
  let updateList = case lookup x stateList of
                     Just n  -> (x, n + 1) : filter (\(y, _) -> y /= x) stateList
                     Nothing -> (x, 1) : stateList
  in ((), updateList)
