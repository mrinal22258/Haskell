...
blendColor (Color r1 g1 b1) (Color r2 g2 b2) = Color (blend r1 r2) (blend g1 g2) (blend b1 b2)
  where
    blend x y = (x + y) `div` 2
...
union (Shape f1) (Shape f2) = Shape (\coord ->
  case (f1 coord, f2 coord) of
    (True, _) -> True
    (_, True) -> True
    _ -> False
  )
...
cut (Shape f1) (Shape f2) = Shape (\coord ->
  case (f1 coord, f2 coord) of
    (True, False) -> True
    _ -> False
  )
...
paintSolid color shape (Picture base) = Picture f
  where
    f coord = case contains shape (getX coord) (getY coord) of
      True -> color
      False -> base coord
...
paint (Picture patFunc) shape (Picture baseFunc) = Picture paintedFunc
  where
    paintedFunc = \(Coord x y) ->
      let inShape = contains shape x y
          patColor = patFunc (Coord x y)
          baseColor = baseFunc (Coord x y)
      in if inShape then patColor else baseColor
...
instance Transform Zoom where
  apply (Zoom factor) (Picture f) = Picture (\(Coord x y) -> f (Coord (x `div` factor) (y `div` factor)))
...
instance Transform Blur where
  apply Blur (Picture p) = Picture f
    where
      ...
      avgColor [] = black  
      avgColor cs =
        let (totalR, totalG, totalB, numColors) = foldl accumulate (0, 0, 0, 0) cs
        in Color (totalR `div` numColors) (totalG `div` numColors) (totalB `div` numColors)

      accumulate :: (Int, Int, Int, Int) -> Color -> (Int, Int, Int, Int)
      accumulate (totalR, totalG, totalB, count) (Color r g b) =
        (totalR + r, totalG + g, totalB + b, count + 1)

      neighbours :: Coord -> [Coord]
      neighbours (Coord x y) =
        map (\(dx, dy) -> Coord (x + dx) (y + dy)) [(0, 0), (1, 0), (-1, 0), (0, 1), (0, -1)]

      f :: Coord -> Color
      f coord = avgColor $ map p (neighbours coord)
