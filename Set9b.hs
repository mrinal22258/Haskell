...
prettyPrint n queens = foldr (\r acc -> rowString r ++ "\n" ++ acc) "" [1..n]
  where
    rowString r = [if (r, c) `elem` queens then 'Q' else '.' | c <- [1..n]]
...
sameRow (i, _) (k, _) = case i == k of
                          True -> True
                          False -> False

sameCol :: Coord -> Coord -> Bool
sameCol (_, j) (_, l) = case j == l of
                          True -> True
                          False -> False

sameDiag :: Coord -> Coord -> Bool
sameDiag (i, j) (k, l) = case i - j == k - l of
                           True -> True
                           False -> False

sameAntidiag :: Coord -> Coord -> Bool
sameAntidiag (i, j) (k, l) = case i + j == k + l of
                               True -> True
                               False -> False
...
danger (x, y) stack = any isThreatening stack
  where
    isThreatening (i, j) = sameRowThreat || sameColThreat || sameDiagThreat || sameAntidiagThreat
      where
        sameRowThreat = sameRow (x, y) (i, j)
        sameColThreat = sameCol (x, y) (i, j)
        sameDiagThreat = sameDiag (x, y) (i, j)
        sameAntidiagThreat = sameAntidiag (x, y) (i, j)
...
prettyPrint2 size queens = foldr (\r acc -> foldr (\c rowAcc -> markSquare (r, c) : rowAcc) "\n" [1..size] ++ acc) "" [1..size]
  where
    markSquare coord
      | coord `elem` queens = 'Q'  
      | danger coord queens = '#' 
...
fixFirst n ((i,j):rest)
  | j > n = Nothing  
  | danger (i, j) rest = fixFirst n ((i, j + 1) : rest)  
...
finish n (q : qs)
  | length qs == n = qs
  | otherwise = finish n $ step n (q : qs)
