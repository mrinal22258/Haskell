...
balance db account = do
  rows <- query db balanceQuery [account]
  let amounts = map (\[x] -> x) rows :: [Int]
  return $ sum amounts
...
parseCommand (action : account : amount) =
  case (T.unpack action, amount) of
    ("deposit", [amt]) -> Deposit account <$> parseInt amt
    ("balance", []) -> Just (Balance account)
    _ -> Nothing
...
perform _ Nothing = error "Unexpected Nothing value in perform" 
