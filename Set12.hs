...
instance Functor List where
  fmap f = foldr (\x acc -> LNode (f x) acc) Empty
    where
      foldr _ acc Empty = acc
      foldr g acc (LNode x xs) = g x (foldr g acc xs)
...
count x = getSum . foldMap (\y -> if y == x then Sum 1 else Sum 0)
...
inBoth xs ys = foldMap (\x -> if x `elem` toList ys then [x] else []) xs
...
instance Foldable List where
  foldr _ z Empty = z
  foldr f z (LNode x rest) = f x (foldr f z rest)
