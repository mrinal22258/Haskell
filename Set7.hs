...
step state event = case (state, event) of
  (Start, AddEggs) -> AddingEggs
  (AddingEggs, AddFlour) -> AddingFlour
  (AddingEggs, AddSugar) -> AddingSugar
  (AddingFlour, AddSugar) -> NeedsMixing
  (AddingSugar, AddFlour) -> NeedsMixing
  (NeedsMixing, Mix) -> NeedsBaking
  (NeedsBaking, Bake) -> Finished
  (Finished, _) -> Finished
  (_, _) -> Error 
...
operation _   = error "Unsupported operation"

evaluate :: Arithmetic -> Integer
evaluate expr =
  if isLiteral expr
    then getLiteralValue expr
    else if isAddition expr
           then evaluateAddition expr
           else evaluateMultiplication expr
  where
    isLiteral (Literal _) = True
    isLiteral _ = False

    getLiteralValue (Literal n) = n
    getLiteralValue _ = error "Not a literal"

    isAddition (Addition _ _) = True
    isAddition _ = False

    evaluateAddition (Addition e1 e2) = evaluate e1 + evaluate e2
    evaluateAddition _ = error "Not an addition"

    evaluateMultiplication (Multiplication e1 e2) = evaluate e1 * evaluate e2
    evaluateMultiplication _ = error "Not a multiplication"

render :: Arithmetic -> String
render expr =
  if isLiteral expr
    then show (getLiteralValue expr)
    else if isAddition expr
           then "(" ++ renderAddition expr ++ ")"
           else "(" ++ renderMultiplication expr ++ ")"
  where
    isLiteral (Literal _) = True
    isLiteral _ = False
    ...
    renderAddition (Addition e1 e2) = render e1 ++ "+" ++ render e2
    renderAddition _ = error "Not an addition"

    renderMultiplication (Multiplication e1 e2) = render e1 ++ "*" ++ render e2
    renderMultiplication _ = error "Not a multiplication"
