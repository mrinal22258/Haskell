...
greet :: String -> IO ()
greet name = printLines ["HELLO " ++ name]
    where
        printLines :: [String] -> IO ()
        printLines = mapM_ putStrLn
...
readUntil f = do
  word <- getLine
  let stop = f word
  rest <- getRest stop word
  return rest
  where
    getRest stop word = do
      whenM (return stop) (return ())
      if stop
        then return []
        else do
          rest <- readUntil f
...
countdownPrint n = printLines $ countdown n
  where
    countdown 0 = ["0"]
    countdown k = show k : countdown (k - 1)
    printLines :: [String] -> IO ()
    printLines = mapM_ putStrLn
...
isums :: Int -> IO Int
isums n = processNumbers n 0
  where
    processNumbers :: Int -> Int -> IO Int
    processNumbers 0 total = return total  
    processNumbers k total = do
        num <- readLn  
        let newTotal = total + num  
        putStrLn $ show newTotal  
        processNumbers (k - 1) newTotal  
