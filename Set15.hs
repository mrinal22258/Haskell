...
statements xs ys = generateStatements xs ys

generateStatements :: [String] -> [String] -> [String]
generateStatements xs ys = liftA2 combineStatements xs ys >>= id

combineStatements :: String -> String -> [String]
combineStatements x y = [x ++ " is " ++ y, x ++ " is not " ++ y]
...
calculator "negate" numStr = case readMaybe numStr of
    Just n -> Just (negate n)
    Nothing -> Nothing
calculator "double" numStr = case readMaybe numStr of
    Just n -> Just (n * 2)
    Nothing -> Nothing
...
validateDiv num den = check (den /= 0) "Division by zero!" $ num `div` den
...
boolOrInt s = validBool <|> validInt
  where
    validBool = case readMaybe s :: Maybe Bool of
      Just x -> pure (Left x)
      Nothing -> invalid "Not a Bool"
    validInt = case readMaybe s :: Maybe Int of
      Just x -> pure (Right x)
