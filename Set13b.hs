...
path maze place1 place2 =
  elem place2 $ execState (visit maze place1) []
...
allSums :: [Int] -> [Int]
allSums xs =
  foldr (\x acc -> acc ++ map (x +) acc) [0] xs
...
f2 acc x = state $ \visited ->
  if x `elem` visited
    then (acc, visited)
    else (acc + x, x : visited)
...
instance Functor Result where
  fmap f result = 
    ...
    then let Failure msg = result in Failure msg 
    else let MkResult x = result in MkResult (f x)

isNoResult :: Result a -> Bool
...
mkCounter = do
  countRef <- newIORef 0
  pendingRef <- newIORef 0
  let inc = modifyIORef' pendingRef (+1)
  let get = do
        pending <- readIORef pendingRef
        modifyIORef' countRef (+ pending)
        writeIORef pendingRef 0
        readIORef countRef
  return (inc, get)
