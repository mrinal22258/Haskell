...
distinct [] = True
distinct (x:xs) = not (elementExists x xs) && distinct xs
      where 
...
rangeOf [] = error "Empty list"
rangeOf xs = maxElement xs - minElement xs
      where 
            minElement :: (Ord a) => [a] -> a
            minElement [] = error "Empty list"
            minElement [x] = x
...
getScore :: Map.Map String Int -> String -> Int
getScore scores player = Map.findWithDefault 0 player scores
winner :: Map.Map String Int -> String -> String -> String
winner scores player1 player2
  | score1 >= score2 = player1
  | otherwise        = player2
  where
    score1 = getScore scores player1
    score2 = getScore scores player2
...
freqs xs = foldr updateFreq Map.empty xs
      where 
           updateFreq :: (Ord a) => a -> Map.Map a Int -> Map.Map a Int
...
transfer from to amount bank
  | amount < 0 = bank
  | findBalance to < 0 = bank
  | findBalance from < amount = bank
  | otherwise = adjustBalance amount to (adjustBalance (- amount) from bank)
  where
    findBalance acc = Map.findWithDefault (- 1) acc bank
    adjustBalance sum acc bank = Map.adjust (+ sum) acc bank
...
getElem :: Ix i => i -> Array i a -> a
getElem idx arr = arr ! idx

setElem :: Ix i => i -> a -> Array i a -> Array i a
setElem idx newVal arr = arr // [(idx, newVal)]

swapElems :: Ix i => i -> i -> Array i a -> Array i a
swapElems i j arr = let
    elemI = getElem i arr
    elemJ = getElem j arr
    arr'  = setElem i elemJ arr
    arr'' = setElem j elemI arr'
  in arr''
...
arrayAssocs :: Ix i => Array i a -> [(i, a)]
arrayAssocs arr = assocs arr

findMax :: (Ix i, Ord a) => [(i, a)] -> i
findMax = fst . maximumBy (comparing snd)

maxIndex :: (Ix i, Ord a) => Array i a -> i
maxIndex = findMax . arrayAssocs
