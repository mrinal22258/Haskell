...
swapIORefs ref1 ref2 = do
    val1 <- readIORef ref1
    val2 <- readIORef ref2
    modifyIORef ref1 (\_ -> val2)
    modifyIORef ref2 (\_ -> val1)
...
hFetchLines handle = compose (return . lines) hGetContents handle
...
hSelectLines h nums = do
    ...
    return (selectLines allLines nums)
        where
            selectLines :: [String] -> [Int] -> [String]
            selectLines lines indexes = mapMaybe (`safeIndex` lines) indexes
                where
                    safeIndex :: Int -> [a] -> Maybe a
                    safeIndex i xs
                        | i > 0 && i <= length xs = Just (xs !! (i - 1))
                        | otherwise = Nothing
...
interact' f initialState = interactLoop initialState
  where
    interactLoop state =
      getLine >>= \input ->  
